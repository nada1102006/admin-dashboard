  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 text-slate-900 bg-slate-50/50 dark:bg-slate-900 dark:text-slate-200">
      <div className="mx-auto max-w-[1600px] space-y-6 lg:space-y-8 dark:bg-slate-900 dark:text-slate-200">
            ...
            <div className="flex gap-3">
              <button
                ...
                className={`flex h-13 items-center justify-center gap-2 rounded-2xl border px-6 text-[15px] font-semibold transition-colors ${
                  filtersOpen 
                    ? "border-[#00bad5] bg-[#e0f7fc] text-[#008ba0] dark:bg-[#00bad5]/20 dark:text-[#00bad5]" 
        ...
        {error && (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-[15px] font-medium text-rose-700 dark:border-rose-900/50 dark:bg-rose-900/20 dark:text-rose-400">
            {error}
          </div>
        )}
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-110 animate-pulse rounded-4xl border border-slate-200/50 bg-slate-100 dark:border-slate-800 dark:bg-slate-800/50" />
            ))}
          </div>
        ) : products.length > 0 ? (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-8 pb-4">
                <button 
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                >
                  <FiChevronLeft />
                </button>
          ...
          <div className="flex flex-col items-center justify-center py-20 rounded-4xl border border-dashed border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
...
function StatCard({ icon, value, label, tone, className = "" }) {
  const tones = {
    cyan: "bg-[#ccf0f6] text-[#008ba0] dark:bg-[#00bad5]/20 dark:text-[#00bad5]",
    amber: "bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400",
    emerald: "bg-[#dcfce7] text-[#16a34a] dark:bg-emerald-900/30 dark:text-emerald-400",
    rose: "bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400",
  };

  return (
    <div className={`flex flex-col rounded-[28px] border border-slate-100 bg-white p-6 shadow-[0_2px_20px_rgba(0,0,0,0.02)] transition-shadow hover:shadow-md dark:bg-slate-900 dark:border-slate-800 ${className}`}>
      <div className={`mb-5 flex h-12 w-12 items-center justify-center rounded-2xl text-xl ${tones[tone]}`}>
        {icon}
      </div>
      <h3 className="text-[32px] font-black tracking-tight text-[#121926] leading-none dark:text-white">{value}</h3>
      <p className="mt-2 text-[14px] font-medium text-slate-500 dark:text-slate-400">{label}</p>
    </div>
  );
}
...
function ProductCard({ product, isDeleting, onDelete, onQuickEdit }) {
  ...
  return (
    <div className={`group relative flex flex-col overflow-hidden rounded-4xl border border-slate-100 bg-white shadow-[0_2px_20px_rgba(0,0,0,0.02)] transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:border-slate-800 dark:bg-slate-900 ${isDeleting ? "opacity-60 grayscale" : ""}`}>
      {/* Image Container */}
      ...
      <div className="flex flex-1 flex-col px-6 pb-6 pt-5 dark:bg-slate-900">
        <Link to={`/products/${product._id}`} className="text-[19px] font-extrabold text-[#121926] transition-colors hover:text-[#00bad5] line-clamp-1 dark:text-white">
          {product.name}
        </Link>
        ...
        <div className="mt-auto pt-5">
          <div className="flex items-baseline gap-2">
            <span className="text-[32px] font-black tracking-tight text-[#121926] dark:text-white">
              {(Number(product.price) || 0).toFixed(0)} <span className="text-[16px] font-bold text-slate-500 tracking-normal ml-1">EGP</span>
            </span>
            {(Number(product.discountPrice) || Number(product.discount) || 0) > 0 && (
              <span className="text-[14px] font-semibold text-[#16a34a] dark:text-emerald-400">
                -{(Number(product.discountPrice) || Number(product.discount) || 0).toFixed(0)} EGP off
              </span>
            )}
          </div>
          ...
          <div className="mt-3.5 flex flex-wrap gap-2">
            {product.tags?.map((tag, index) => (
              <span key={`${tag}-${index}`} className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-1 text-[12px] font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
                {tag}
              </span>
            ))}
          </div>
