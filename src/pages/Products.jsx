...
//   return (
//     <div className="min-h-screen p-4 sm:p-6 lg:p-8 text-slate-900 bg-slate-50/50 dark:bg-slate-900 dark:text-slate-200 ">
//       <div className="mx-auto max-w-[1600px] space-y-6 lg:space-y-8 dark:bg-slate-900 dark:text-slate-200">
//         
//         {/* Header Card */}
//         <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 rounded-[28px] bg-white shadow-[0_2px_20px_rgba(0,0,0,0.02)] gap-4 border border-[#e0f7fc]/50 dark:bg-slate-900 dark:text-white">
//           <div className="flex items-center gap-5">
//             <div className="w-15 h-15 shrink-0 flex items-center justify-center bg-[#ccf0f6] rounded-[20px]  dark:text-white">
...
//         {/* Search & Filters */}
//         <div className="rounded-[28px] bg-white p-5 lg:p-6 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-slate-100 dark:bg-slate-900 dark:text-white">
...
//                 className="h-13 w-full rounded-2xl border border-slate-200 bg-[#fafdfd] pl-12 pr-4 text-[15px] text-[#121926] outline-none transition-all focus:border-[#00bad5] focus:ring-1 focus:ring-[#00bad5] placeholder:text-slate-400 dark:bg-slate-900 dark:text-white"
//               />
//             </div>
...
//                     : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-700"
//                 }`}
//               >
...
//                     className="h-[52px] w-full rounded-[16px] border border-slate-200 bg-white px-4 text-[15px] text-slate-700 outline-none transition-colors hover:border-slate-300 focus:border-[#00bad5] focus:ring-1 focus:ring-[#00bad5] dark:bg-slate-900 dark:text-white"
//                   >
...
//                     className="h-13 w-full rounded-2xl border border-slate-200 bg-white px-4 text-[15px] text-slate-700 outline-none transition-colors hover:border-slate-300 focus:border-[#00bad5] focus:ring-1 focus:ring-[#00bad5] placeholder:text-slate-400 dark:bg-slate-900 dark:text-white" 
//                   />
//                 </FilterField>
//               </div>
//             </div>
...
//                 onQuickEdit={() => { setSelectedProduct(product); setIsQuickEditOpen(true); }}
//                 />
//               ))}
//             </div>
//             
...
//   return (
//     <div className="flex flex-col rounded-[28px] border border-slate-100 bg-white p-6 shadow-[0_2px_20px_rgba(0,0,0,0.02)] transition-shadow hover:shadow-md" className={`p-4 rounded-lg ${tones[tone]} ${className}`}>
...
//     <span key={`${tag}-${index}`} className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-1 text-[12px] font-medium text-slate-600">
//       {tag}
//     </span>
//   ))}
// </div>
...
//      
//         <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5 dark:border-slate-900">
...
//               type="button"
//               onClick={onQuickEdit}
//               className="flex items-center gap-1.5 rounded-[14px] border border-slate-100 bg-white px-3.5 py-2 text-[12px] font-semibold text-slate-600 transition-colors  hover:border-slate-200 shadow-sm dark:bg-slate-900 dark:text-white"
//             >
...
//             disabled={isDeleting}
//             onClick={onDelete}
//             className="flex items-center gap-1.5 rounded-[14px] border border-rose-100 bg-rose-50 px-3.5 py-2 text-[12px] font-semibold text-rose-600 transition-colors  disabled:opacity-50 dark:bg-slate-900 dark:text-white"
//           >
//             <FiTrash2 size={14} />
...
// function ActionBtn({ icon, label, to, className = "", tone = "default" }) {
//   const content = (
//     <>
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 text-slate-900 bg-slate-50/50 dark:bg-slate-900 dark:text-slate-200">
      <div className="mx-auto max-w-[1600px] space-y-6 lg:space-y-8 dark:bg-slate-900 dark:text-slate-200">
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-110 animate-pulse rounded-4xl border border-slate-200/50 bg-slate-100" />
            ))}
          </div>
        ) : products.length > 0 ? (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-8 pb-4">
                <button 
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-50"
                >
                  <FiChevronLeft />
                </button>
                <span className="text-sm font-semibold text-slate-600 px-4">Page {page} of {totalPages}</span>
          ...
          <div className="flex flex-col items-center justify-center py-20 rounded-4xl border border-dashed border-slate-200 bg-white">
            <div className="w-20 h-20 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 mb-4">
              <FiPackage size={32} />
            </div>
            <h3 className="text-xl font-bold text-slate-900 mb-2">No products found</h3>
...
// ============================================
// FIXED StatCard - Removed duplicate className
// ============================================
function StatCard({ icon, value, label, tone, className = "" }) {
  const tones = {
    cyan: "bg-[#ccf0f6] text-[#008ba0]",
  ...
  return (
    <div className={`flex flex-col rounded-[28px] border border-slate-100 bg-white p-6 shadow-[0_2px_20px_rgba(0,0,0,0.02)] transition-shadow hover:shadow-md ${className}`}>
      <div className={`mb-5 flex h-12 w-12 items-center justify-center rounded-2xl text-xl ${tones[tone]}`}>
        {icon}
      </div>
      <h3 className="text-[32px] font-black tracking-tight text-[#121926] leading-none">{value}</h3>
      <p className="mt-2 text-[14px] font-medium text-slate-500">{label}</p>
    </div>
  );
}
...
function ProductCard({ product, isDeleting, onDelete, onQuickEdit }) {
  ...
  return (
    <div className={`group relative flex flex-col overflow-hidden rounded-4xl border border-slate-100 bg-white shadow-[0_2px_20px_rgba(0,0,0,0.02)] transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] ${isDeleting ? "opacity-60 grayscale" : ""}`}>
      {/* Image Container */}
      <div className="relative aspect-4/3 w-full shrink-0 overflow-hidden bg-slate-50">
        <ImageSlider images={product.images?.map(img => img.url || img)} altText={product.name} />
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5 dark:border-slate-900">
          <div className="flex flex-wrap gap-2">
            ...
            <button 
              type="button"
              onClick={onQuickEdit}
              className="flex items-center gap-1.5 rounded-[14px] border border-slate-100 bg-white px-3.5 py-2 text-[12px] font-semibold text-slate-600 transition-colors hover:border-slate-200 hover:bg-slate-50 shadow-sm dark:bg-slate-900 dark:text-white"
            >
              <FiSliders size={14} /> Quick Edit
            </button>
          </div>
